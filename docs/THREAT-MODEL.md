...
| T12 | dados chegam ao clipboard ou rede                                | exfiltração                       | campos KDBX protegidos não retornam ao React; valores demonstrativos usam limpeza condicional por fingerprint; nenhuma chamada de rede/telemetria | comparação e limpeza não são atômicas; fechamento e permissões podem impedir limpeza; OS e outras aplicações estão fora do controle |
